...
def cv_difference(directory):
    ...
    sub_bw_anti, bw_anti, bw_anti_max_volt,legend1= calculate_graph_data(paths[3:6])
    sub_bw_noanti, bw_noanti, bw_noanti_max_volt,legend2 = calculate_graph_data(paths[6:9])
    sub_pk_anti, pk_anti, pk_anti_max_volt,legend3 = calculate_graph_data(paths[10:12])
    sub_pk_noanti, pk_noanti, pk_noanti_max_volt,legend4 = calculate_graph_data(paths[12:15])

    diff_bw = np.subtract(sub_bw_anti[1], sub_bw_noanti[1])
    diff_bw_std = np.sqrt(np.add(np.square(sub_bw_anti[2]), np.square(sub_bw_noanti[2])))
    diff_pk = np.subtract(sub_pk_anti[1], sub_pk_noanti[1])
    diff_pk_std = np.sqrt(np.add(np.square(sub_pk_anti[2]), np.square(sub_pk_noanti[2])))

    return [sub_bw_anti[0], diff_bw, diff_bw_std, diff_pk, diff_pk_std]

def plot_cv(directory):
    ...
    return [bw_noanti_3[0],bw_var,pk_var]
